"""
消融：mpnet + bge_large + XGB+LR — PCA 維度影響

在 ablation_exp016 中，mpnet + bge_large + XGB+LR + PCA 32 = 0.8800（最佳）。
ablation_pca_3model 顯示 3-model + no DR = 0.8800（PCA 反而有損）。
本腳本測試 2-model 最佳組合在不同 DR 下是否能超越 0.8800。

條件：no DR / PCA 128 / PCA 64 / PCA 32（對照）/ PCA 16
"""
...
def main():
    ...
    # 只 encode mpnet + bge_large
    bm_cfgs = [bm for bm in cfg['stacking']['base_models'] if bm['id'] in TARGET_MODELS]
    print(f'>> Models: {[b["id"] for b in bm_cfgs]}')
